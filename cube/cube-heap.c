...
#define DESCRIPTOR_HEAP_COUNT 3
...
enum {
 ...
 #define APP_NAME_STR_LEN 80
     ...
     // related variable for descriptor heap
     uint32_t descriptorHeapMode;
 ...
 static void demo_draw_build_cmd(struct demo *demo, VkCommandBuffer cmd_buf) {
     ...
     else
     {
        ...
        if((vkCmdBindBufferHeapKHR == NULL) || (vkCmdBindImageHeapKHR == NULL) || (vkCmdBindSamplerHeapKHR == NULL))
        {
            fprintf(stderr, "Fail to get descriptor heap bind vulkan api from vulkan driver!\n");
        }

        // bind buffer heap buffer to cmd buffer
        VkBufferDeviceAddressInfo bufferAddressInfo = {
            .sType = VK_STRUCTURE_TYPE_BUFFER_DEVICE_ADDRESS_INFO,
            .pNext = NULL,
            .buffer = demo->swapchain_image_resources[demo->current_buffer].descriptor_heap[bufferHeap],
        };

        VkDeviceAddressRangeKHR bufferHeapRange = {
            .address = vkGetBufferDeviceAddressKHR(demo->device, &bufferAddressInfo),
            .size = demo->descriptorHeapProperties.bufferHeapAlignment,
        };

        VkBindHeapInfoKHR pBufferBindInfo = {
            .sType = VK_STRUCTURE_TYPE_BIND_HEAP_INFO_KHR,
            .pNext = NULL,
            .heapRange = bufferHeapRange,
            .reservedRangeOffset = 0,
        };

        vkCmdBindBufferHeapKHR(cmd_buf, &pBufferBindInfo);

        // bind image heap buffer to cmd buffer
        VkBufferDeviceAddressInfo ImageAddressInfo = {
            .sType = VK_STRUCTURE_TYPE_BUFFER_DEVICE_ADDRESS_INFO,
            .pNext = NULL,
            .buffer = demo->swapchain_image_resources[demo->current_buffer].descriptor_heap[imageHeap],
        };

        VkDeviceAddressRangeKHR imageHeapRange = {
            .address = vkGetBufferDeviceAddressKHR(demo->device, &ImageAddressInfo),
            .size = demo->descriptorHeapProperties.samplerHeapAlignment,
        };

        VkBindHeapInfoKHR pImageBindInfo = {
            .sType = VK_STRUCTURE_TYPE_BIND_HEAP_INFO_KHR,
            .pNext = NULL,
            .heapRange = imageHeapRange,
            .reservedRangeOffset = imageHeap * demo->descriptorHeapProperties.imageHeapDescriptorAlignment,
        };

        vkCmdBindImageHeapKHR(cmd_buf, &pImageBindInfo);

        // bind sampler heap buffer to cmd buffer
        VkBufferDeviceAddressInfo samplerAddressInfo = {
            .sType = VK_STRUCTURE_TYPE_BUFFER_DEVICE_ADDRESS_INFO,
            .pNext = NULL,
            .buffer = demo->swapchain_image_resources[demo->current_buffer].descriptor_heap[samplerHeap],
        };

        VkDeviceAddressRangeKHR samplerHeapRange = {
            .address = vkGetBufferDeviceAddressKHR(demo->device, &samplerAddressInfo),
            .size = demo->descriptorHeapProperties.samplerHeapAlignment,
        };

        VkBindHeapInfoKHR pSamplerBindInfo = {
            .sType = VK_STRUCTURE_TYPE_BIND_HEAP_INFO_KHR,
            .pNext = NULL,
            .heapRange = samplerHeapRange,
            .reservedRangeOffset = samplerHeap * demo->descriptorHeapProperties.samplerHeapDescriptorAlignment,
        };

        vkCmdBindSamplerHeapKHR(cmd_buf, &pSamplerBindInfo);
     }
 ...
 static void demo_prepare_vs(struct demo *demo) {
     const uint32_t vs_cube_code[] = {
 #include "cube.vert.inc"
     };

     const uint32_t vs_heap_pass_code[] = {
 #include "cube.vert.heap.inc"
     };

     const uint32_t vs_legacy_pass_code[] = {
 #include "cube.vert.legacy.inc"
     };

     const uint32_t* vs_code = NULL;
     uint32_t size = 0;

     if (demo->descriptorHeapMode == 0)
     {
        vs_code = vs_cube_code;
        size    = sizeof(vs_cube_code);
     }
     else if (demo->descriptorHeapMode == 1)
     {
        vs_code = vs_heap_pass_code;
        size    = sizeof(vs_heap_pass_code);
     }
     else if (demo->descriptorHeapMode == 2)
     {
        vs_code = vs_legacy_pass_code;
        size    = sizeof(vs_legacy_pass_code);
     }

     demo->vert_shader_module = demo_prepare_shader_module("cube.vert", demo, vs_code, size);
 }
 
 static void demo_prepare_fs(struct demo *demo) {

     const uint32_t fs_cube_code[] = {
 #include "cube.frag.inc"
     };

     const uint32_t fs_heap_pass_code[] = {
 #include "cube.frag.heap.inc"
     };

     const uint32_t fs_legacy_pass_code[] = {
 #include "cube.frag.legacy.inc"
     };

     const uint32_t* fs_code = NULL;
     uint32_t size = 0;

     if (demo->descriptorHeapMode == 0)
     {
        fs_code = fs_cube_code;
        size    = sizeof(fs_cube_code);
     }
     else if (demo->descriptorHeapMode == 1)
     {
        fs_code = fs_heap_pass_code;
        size    = sizeof(fs_heap_pass_code);
     }
     else if (demo->descriptorHeapMode == 2)
     {
        fs_code = fs_legacy_pass_code;
        size    = sizeof(fs_legacy_pass_code);
     }

     demo->frag_shader_module = demo_prepare_shader_module("cube.frag", demo, fs_code, size);
 }
 ...
 #define NUM_DYNAMIC_STATES 2 /*Viewport + Scissor*/
     ...
     VkDescriptorSetAndBindingMappingKHR mappings[3] = {};
     VkShaderDescriptorSetAndBindingMappingInfoKHR shaderDescriptorSet_vs = {};
     VkShaderDescriptorSetAndBindingMappingInfoKHR shaderDescriptorSet_fs = {};

     if (demo->descriptorHeapMode == 2)
     {
        // uniform buffer layout(set = 0, binding = 0) uniform buf
        // set 0, binding 2 with offset heapindex + (bingding - firstbinding) + index
        // 0 +（0 - 0）+ 0 = 0；
        mappings[0].sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_AND_BINDING_MAPPING_KHR;
        mappings[0].pNext = NULL;
        mappings[0].descriptorSet = 0;
        mappings[0].bindingCount = 1;
        mappings[0].firstBinding = 0;
        mappings[0].source = VK_DESCRIPTOR_MAPPING_SOURCE_HEAP_WITH_CONSTANT_INDEX_KHR;
        mappings[0].sourceData.constantIndex.heapIndex = 0;
        mappings[0].sourceData.constantIndex.pEmbeddedSampler = NULL;
        mappings[0].sourceData.constantIndex.useCombinedImageSamplerIndex = VK_FALSE;
        mappings[0].sourceData.constantIndex.samplerHeapIndex = 0;

        // set 0, binding 2 with offset heapindex + (bingding - firstbinding) + index
        // 0 +（1 - 1）+ 0 = 0；
        mappings[1].sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_AND_BINDING_MAPPING_KHR;
        mappings[1].pNext = NULL;
        mappings[1].descriptorSet = 0; // combined image and sampler
        mappings[1].bindingCount = 1;
        mappings[1].firstBinding = 1;
        mappings[1].source = VK_DESCRIPTOR_MAPPING_SOURCE_HEAP_WITH_CONSTANT_INDEX_KHR;
        mappings[1].sourceData.constantIndex.heapIndex = 0;
        mappings[1].sourceData.constantIndex.useCombinedImageSamplerIndex = VK_FALSE;
        mappings[1].sourceData.constantIndex.samplerHeapIndex = 0;
        mappings[1].sourceData.constantIndex.pEmbeddedSampler = NULL;

        // set 0, binding 2 with offset heapindex + (bingding - firstbinding) + index
        // 0 +（2 - 2）+ 0 = 0；
        mappings[2].sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_AND_BINDING_MAPPING_KHR;
        mappings[2].pNext = NULL;
        mappings[2].descriptorSet = 0; // combined image and sampler
        mappings[2].bindingCount = 1;
        mappings[2].firstBinding = 2;
        mappings[2].source = VK_DESCRIPTOR_MAPPING_SOURCE_HEAP_WITH_CONSTANT_INDEX_KHR;
        mappings[2].sourceData.constantIndex.heapIndex = 0;
        mappings[2].sourceData.constantIndex.useCombinedImageSamplerIndex = VK_FALSE;
        mappings[2].sourceData.constantIndex.samplerHeapIndex = 0;
        mappings[2].sourceData.constantIndex.pEmbeddedSampler = NULL;

        shaderDescriptorSet_vs.sType = VK_STRUCTURE_TYPE_SHADER_DESCRIPTOR_SET_AND_BINDING_MAPPING_INFO_KHR;
        shaderDescriptorSet_vs.pNext = NULL;
        shaderDescriptorSet_vs.mappingCount = 1;
        shaderDescriptorSet_vs.pMappings = mappings;

        shaderDescriptorSet_fs.sType = VK_STRUCTURE_TYPE_SHADER_DESCRIPTOR_SET_AND_BINDING_MAPPING_INFO_KHR;
        shaderDescriptorSet_fs.pNext = NULL;
        shaderDescriptorSet_fs.mappingCount = 2;
        shaderDescriptorSet_fs.pMappings = &mappings[1];
     }
     ...
     shaderStages[0].pNext = (demo->descriptorHeapMode == 2) ? &shaderDescriptorSet_vs : NULL;
     ...
     shaderStages[1].pNext = (demo->descriptorHeapMode == 2) ? &shaderDescriptorSet_fs : NULL;
...
// prepare descriptor heap buffer  
static void demo_prepare_descriptor_heap_buffer(struct demo *demo) 
{
    ...
    if((vkWriteBufferDescriptorsKHR == NULL) || (vkWriteSamplerDescriptorsKHR == NULL) || (vkWriteImageDescriptorsKHR == NULL))
    {
        fprintf(stderr, "Fail to get descriptor heap write vulkan api from vulkan driver!\n");
    }
    ...
    for (unsigned int i = 0; i < demo->swapchainImageCount; i++) 
    {
        for (unsigned int ii = 0; ii < heapCount; ii++)
        {
            ...
            // Write resource srd to heap buffer.
            if (ii == bufferHeap)
            {
            ...
            else if (ii == samplerHeap)
            {
                ...
                err = vkWriteSamplerDescriptorsKHR(demo->device, 1, &sampler, &bufferRagge);
            }
 ...
 static void demo_prepare(struct demo *demo) {
     ...
     {
         demo_prepare_descriptor_heap_buffer(demo);
     }
 ...
 static void demo_cleanup(struct demo *demo) {
     ...
     if (!demo->is_minimized) {
         ...
         for (i = 0; i < demo->swapchainImageCount; i++) {
             ...
             {
                vkDestroyBuffer(demo->device, demo->swapchain_image_resources[i].descriptor_heap[j], NULL);
                vkFreeMemory(demo->device, demo->swapchain_image_resources[i].descHeapMemory[j], NULL);
             }
         }
 ...
 static void demo_select_physical_device(struct demo *demo) {
     ...
     if (device_extension_count > 0) {
         ...
         for (uint32_t i = 0; i < device_extension_count; i++) {
             ...
             if ((!strcmp(VK_KHR_DESCRIPTOR_HEAP_EXTENSION_NAME, device_extensions[i].extensionName)) && (demo->descriptorHeapMode != 0))
             {
                 demo->extension_names[demo->enabled_extension_count++] = VK_KHR_DESCRIPTOR_HEAP_EXTENSION_NAME;
             }
             // VK_KHR_buffer_device_address
             if ((!strcmp(VK_KHR_BUFFER_DEVICE_ADDRESS_EXTENSION_NAME, device_extensions[i].extensionName)) && (demo->descriptorHeapMode != 0))
             {
 ...
 static void demo_init(struct demo *demo, int argc, char **argv) {
     ...
     for (int i = 1; i < argc; i++) {
         ...
         if (strcmp(argv[i], "--descriptor_heap") == 0  && (i < argc - 1))
         {
            demo->descriptorHeapMode = atoi(argv[i + 1]);
            i++;

            if (demo->descriptorHeapMode == 1)
            {
                printf("########## NOTE: descriptor heap is working in heap mode ##########\n");
            }
            else if (demo->descriptorHeapMode == 2)
            {
                printf("########## NOTE: descriptor heap is working in legacy mode ##########\n");
            }
         ...
         char *message =
             ...
             "\t[--descriptor_heap <descriptor heap mode>]\n"
             ...
             "\t<descriptor heap mode>\n"
             "\t\t 0 means descriptor heap is not enable\n"
             "\t\t 1 means heap mode\n"
             "\t\t 2 means legacy mode\n";
