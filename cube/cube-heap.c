...
#define APP_NAME_STR_LEN 80
    ...
    // 0:sampler heap, 1:buffer heap; 2:image heap;
    VkBuffer descHeap[3];

    VkDeviceMemory descHeapMemory[3];
    typedef enum VkHeapType {
        sampler = 0x0,
        buffer  = 0x1,
        image   = 0x2
    };
...
#if DESCRIPTOR_HEAP
void demo_prepare_cube_descriptor_heap(struct demo *demo)
{
...

    {
        ...
        heapInfo.flags = VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT | VK_BUFFER_USAGE_DESCRIPTOR_HEAP_BIT_KHR;
        heapInfo.size  = demo->descriptorHeapProperties.maxSamplerHeapSize;
        err = vkCreateBuffer(demo->device, &heapInfo, NULL, &demo->descHeap[0]);

        // VkDevice device, VkImage image, VkMemoryRequirements* pMemoryRequirements);
        vkGetBufferMemoryRequirements(demo->device, demo->descHeap[0], &memReqs);
        ...
        err = vkAllocateMemory(demo->device, &memAllocInfo, NULL, &demo->descHeapMemory[0]);
        err = vkBindBufferMemory(demo->device, demo->descHeap[0], demo->descHeapMemory[0], 0);

        //heap buffer host ponter
        VkHostAddressStridedRangeKHR bufferRagge = {
        ...
        vkMapMemory(demo->device, demo->descHeapMemory[0], 0, VK_WHOLE_SIZE, 0, &bufferRagge.pData);
        ...
        //     (PFN_vkGetBufferDeviceAddressKHR) vkGetDeviceProcAddr(demo->device, "vkGetBufferDeviceAddressKHR111111111111");
        ...
        for (uint32_t i = 0; i < demo->swapchainImageCount; i++)
        {
            ...
            bufferRagge.pData += bufferRagge.size;
        }

        vkUnmapMemory(demo->device, demo->descHeapMemory[0]);
    }

    {
        ...
        heapInfo.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
        heapInfo.flags = VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT | VK_BUFFER_USAGE_DESCRIPTOR_HEAP_BIT_KHR;
        heapInfo.size  = demo->descriptorHeapProperties.maxBufferHeapSize;
        err = vkCreateBuffer(demo->device, &heapInfo, NULL, &demo->descHeap[1]);

        vkGetBufferMemoryRequirements(demo->device, demo->descHeap[1], &memReqs);
        memAllocInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
        memAllocInfo.pNext = NULL;
        memAllocInfo.allocationSize = memReqs.size;
        memAllocInfo.memoryTypeIndex = 0;
        err = vkAllocateMemory(demo->device, &memAllocInfo, NULL, &demo->descHeapMemory[1]);
        err = vkBindBufferMemory(demo->device, demo->descHeap[1], demo->descHeapMemory[1], 0);
    }

    {
        //create image descriptor heap
        memset(&heapInfo, 0, sizeof(heapInfo));
        memset(&memReqs, 0, sizeof(memReqs));
        memset(&memAllocInfo, 0, sizeof(memAllocInfo));
        ...
        heapInfo.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
        heapInfo.flags = VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT | VK_BUFFER_USAGE_DESCRIPTOR_HEAP_BIT_KHR;
        heapInfo.size  = demo->descriptorHeapProperties.maxImageHeapSize;
        err = vkCreateBuffer(demo->device, &heapInfo, NULL, &demo->descHeap[2]);

        vkGetBufferMemoryRequirements(demo->device, demo->descHeap[2], &memReqs);
        memAllocInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
        memAllocInfo.pNext = NULL;
        memAllocInfo.allocationSize = memReqs.size;
        memAllocInfo.memoryTypeIndex = 0;
        err = vkAllocateMemory(demo->device, &memAllocInfo, NULL, &demo->descHeapMemory[2]);
        err = vkBindBufferMemory(demo->device, demo->descHeap[2], demo->descHeapMemory[2], 0);
    }

    // try write buffer to  buffer heap
